...
def discover_candidates(root: str, category: str, snapshot_date: str, group_depth: int = 2) -> list[dict]:
    """Group category matches under `root` at root_depth+group_depth, with
    totals and the majority byte owner (the provisional assignee)."""
    ...
    rows = get_client().execute(sql, {
        "snap": snapshot_date, "root": root, "rootpfx": root + "/%",
    })
    return [
        {
            "path": grp,
            ...
            "owner_confidence": round(float(conf), 3),
            "suggest_assignment": float(conf) >= 0.6,
        }
